    ...
    <div className="space-y-6 p-4 md:p-8">
      ...
      <Card className="p-6">
        ...
        <form onSubmit={handleSubmit} className="flex items-end gap-4">
          <Input label="Full Name" value={name} onChange={e => setName(e.target.value)} className="flex-grow" />
          <Input label="Email" type="email" value={email} onChange={e => setEmail(e.target.value)} className="flex-grow" />
        ...
        {isLoading ? (
          ...
          <ul className="space-y-3">
            {supervisors?.map((sup: Supervisor) => (
              <li key={sup.id} className="flex justify-between items-center p-3 rounded-lg bg-gray-800/50">
                <div>
                  <p className="font-semibold">{sup.name}</p>
                  <p className="text-sm text-gray-400">{sup.email}</p>
                </div>
                <p className="text-sm">{sup.assignedInterns} interns assigned</p>
              </li>
            ))}
          </ul>
        )}
