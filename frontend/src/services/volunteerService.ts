import { Volunteer, VolunteerOpportunity, Application, VolunteerBadge } from '@/types/volunteer';
...
const mockOpportunities: VolunteerOpportunity[] = [
  ...
  },
  {
    ...
    hours: 60,
    duration: 'Continuous',
    description: 'Assist patients with non-medical needs and provide companionship.',
  },
];
...
export const getApplications = async (volunteerId: string): Promise<Application[]> => {
  await delay(600);
  console.log(`Fetching applications for volunteer ${volunteerId}`);
  return mockApplications;
};

export const applyForOpportunity = async (volunteerId: string, opportunityId: string): Promise<Application> => {
  await delay(1000);
...
export const getBadges = async (volunteerId: string): Promise<VolunteerBadge[]> => {
  await delay(300);
  ...
  if (volunteer && volunteer.stats.totalHours > 100) badges.push('Distinguished Volunteer');
  if (volunteer && volunteer.stats.totalHours > 50) badges.push('Committed');
  badges.push('Active');
  return badges;
};

export const runSoftSkillsTest = async (volunteerId: string): Promise<{ score: number; awardedBadge: VolunteerBadge | null }> => {
  await delay(1500);
  ...
  const awardedBadge: VolunteerBadge | null = score > 90 ? 'Distinguished Volunteer' : null;
